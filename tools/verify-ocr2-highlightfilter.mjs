...
// T4 findRegions + computeLayout：ROI 僅黃區
{
  ...
  // v2 ROI 放大（2026-09-01 元首令）：TARGET_W=600, MAX_SCALE=3。
  // 81px 寬 ROI → scale=min(3,600/81)=3 → 243；31px 高 → 93。
  // 參考：Tesseract 官方建議字高 ≥30px 再辨識（tesseract.js 實測行高閾值），
  // 小 ROI 先放大是 OCR 前處理通用做法（見 upscale.js Otsu+放大管線）。
  check('T4c 併圖 = 243x93（v2 ROI 3x 放大；原圖 200x60）', [lay.outW, lay.outH], [81 * 3, 31 * 3]);
  // 原始幾何釘（maxScale:1）：ROI 本體仍是 81x31，放大只是落地層
  const layRaw = computeLayout(boxes, W, H, { targetW: 1, maxScale: 1 });
  check('T4c-raw 未放大幾何 = 81x31', [layRaw.outW, layRaw.outH], [81, 31]);
  ...
  // v2 同上：16px 塊 → scale 3 → 48；81px 塊 → 243；併圖寬 = 48+243 = 291
  check('T4h 兩塊併圖寬=48+243=291（v2 3x 放大）', lay2.outW, 16 * 3 + 81 * 3);
