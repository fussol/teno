...
import { getToday, toLocalDateStr } from '../../src/core/scheduler.js';
...
const ANKI = {
  dayCutoff: 360, timezoneOffset: 480, newPerDay: 100,
  learnSteps: '1,10', relearnSteps: '10', maxReviewsPerDay: 1000,
  reviewMix: 2, mode: 'flip', learnAheadLimit: 20,
};
const LEARN_STEPS = [1 / 1440, 10 / 1440];
const RELEARN_STEPS = [10 / 1440];
const today = getToday(ANKI.dayCutoff, ANKI.timezoneOffset);
...
function makeSession(s) { return new Session({ ...s, fsrs: new FSRS(), ...ANKI }); }
function localDue(due) {
  if (!due) return '';
  return toLocalDateStr(new Date(due), ANKI.timezoneOffset, ANKI.dayCutoff);
}
...
function cmdDash() {
  ...
  const tz = db.prepare(`SELECT value FROM settings WHERE key='ankiSettings'`).get();
  const anki = tz ? JSON.parse(tz.value) : {};
  const tzOff = anki.timezoneOffset ?? 480;
  const cutoff = db.prepare(`SELECT value FROM settings WHERE key='dayCutoff'`).get();
  const dayCutoff = cutoff ? parseInt(cutoff.value) : 300;
  ...
  for (const e of log2) {
    buttons[e.rating ?? 2]++;
    const h = e.reviewed_at ? new Date(e.reviewed_at).getHours() : 0;
    hours[h]++;
  }
...
function cmdAdd() {
  ...
  backupDb();
  const w = dbw();
  w.prepare(`INSERT INTO words (id, word, definition, part_of_speech, pronunciation, example, deck, tags, image, description, related, forms, synonym, antonym, derivative, examples)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
...
function cmdTheme() {
  ...
  if (sub === 'mode') {
    ...
    const cur = db.prepare(`SELECT value FROM settings WHERE key='themeMode'`).get().value;
    console.log(`themeMode = ${cur}`);
    log('READ', `theme mode = ${cur}`);
  } else if (sub === 'accent') {
    const v = args[1];
    ...
    const cur = db.prepare(`SELECT value FROM settings WHERE key='themeAccent'`).get().value;
    console.log(`themeAccent = ${cur}`);
    log('READ', `theme accent = ${cur}`);
    ...
    const cur = db.prepare(`SELECT value FROM settings WHERE key='themeAccentIntensity'`).get().value;
    console.log(`themeAccentIntensity = ${cur}`);
    log('READ', `theme intensity = ${cur}`);
    ...
    const cur = db.prepare(`SELECT value FROM settings WHERE key='colorPalette'`).get().value;
    console.log(`colorPalette = ${cur}`);
    log('READ', `colorPalette = ${cur}`);
  } else {
...
function cmdTts() {
  ...
  if (sub === 'speed') {
    ...
    const cur = db.prepare(`SELECT value FROM settings WHERE key='ttsSpeed'`).get().value;
    console.log(`ttsSpeed = ${cur}`);
    log('READ', `tts speed = ${cur}`);
  } else if (sub === 'voice') {
    if (v) { writeSetting('ttsVoice', v); log('WRITE', `tts voice=${v}`); }
    const cur = db.prepare(`SELECT value FROM settings WHERE key='ttsVoice'`).get().value;
    console.log(`ttsVoice = ${cur}`);
    log('READ', `tts voice = ${cur}`);
  } else if (sub === 'pitch') {
    if (v !== undefined) {
    ...
    const cur = db.prepare(`SELECT value FROM settings WHERE key='ttsPitch'`).get().value;
    console.log(`ttsPitch = ${cur}`);
    log('READ', `tts pitch = ${cur}`);
  } else if (sub === 'engine') {
    if (v) { writeSetting('ttsEngine', v); log('WRITE', `tts engine=${v}`); }
    const cur = db.prepare(`SELECT value FROM settings WHERE key='ttsEngine'`).get().value;
    console.log(`ttsEngine = ${cur}`);
    log('READ', `tts engine = ${cur}`);
  } else {
...
function cmdDay() {
  ...
  const cur = db.prepare(`SELECT value FROM settings WHERE key='dayCutoff'`).get().value;
...
function cmdAnkiSet() {
  ...
  let nv = value;
  if (['maxIvl','cardsPerDay','leechThreshold','reviewMix','timezoneOffset','learnAheadLimit'].includes(key)) nv = parseInt(value);
...
function cmdExam() {
  const sub = args[0];
  if (sub === 'list') {
    const rows = db.prepare('SELECT * FROM exam_history ORDER BY id DESC LIMIT 30').all();
    console.log(`測驗紀錄 (${rows.length}):`);
    for (const r of rows) console.log(`  ${r.id} ${r.word} ${r.correct ? '✓' : '✗'} ${r.question_type} ${r.examinated_at ?? r.examined_at}`);
...
function cmdExamRun() {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    w.prepare('INSERT INTO exam_history (word, correct, question_type, examined_at) VALUES (?,?,?,datetime(\'now\'))').run(wd.word, isCorrect ? 1 : 0, mode);
...
function cmdExportCsv() {
  ...
  const head = 'word,definition,part_of_speech,pronunciation,example,deck,tags';
...
function cmdRate() {
  ...
  let card = session.cards.get(w.id);
  const isNew = !card;
  if (!card) {
    card = { due: new Date(rateNow).toISOString(), stability: 1, difficulty: 5, elapsedDays: 0, scheduledDays: 0, reps: 0, lapses: 0, state: 0, step: 0, lastReview: null, buried: false, suspended: false, interval: 0 };
  }
  const res = session.fsrs.review(card, rating, Math.random(), LEARN_STEPS, RELEARN_STEPS);
  const dueIso = new Date(rateNow + Math.max(60000, Math.round(res.dueDays * 86400000))).toISOString();
  ...
  if (isNew) {
    wdb.prepare(`INSERT INTO cards (word_id, due, stability, difficulty, elapsed_days, scheduled_days, reps, lapses, state, step, last_review, buried, suspended)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,0,0)`).run(
      w.id, dueIso, res.stability, res.difficulty, 0, sched,
      res.reps, res.lapses, res.state, res.step ?? 0, nowIso);
  } else {
    wdb.prepare(`UPDATE cards SET due=?, stability=?, difficulty=?, scheduled_days=?, reps=?, lapses=?, state=?, step=?, last_review=? WHERE word_id=?`).run(
      dueIso, res.stability, res.difficulty, sched,
      res.reps, res.lapses, res.state, res.step ?? 0, nowIso, w.id);
  }
  ensureSchema();
  // card_state = 複習前狀態, new_state = 複習後狀態 (與 app 一致)
  wdb.prepare(`INSERT INTO review_log (word_id, rating, reviewed_at, mode, card_state, new_state) VALUES (?,?,?,?,?,?)`).run(
    w.id, rating, nowIso, 'flip', isNew ? 0 : (card.state ?? 0), res.state);
  wdb.close();
...
function cmdSim() {
  ...
  while (session.running && i < 50000) {
    ...
    if (!card) {
      card = { due: new Date().toISOString(), stability: 1, difficulty: 5, elapsedDays: 0, scheduledDays: 0, reps: 0, lapses: 0, state: 0, step: 0, lastReview: null, buried: false, suspended: false, interval: 0 };
  ...
  log('READ', `sim: 顯示=${i} 唯一=${seen.size} 重複=${repeat} stray=${stray.length}`);
}
...
function cmdFix() {
  ...
  try {
    if (sub === 'reset-card' && id) {
      w.prepare(`UPDATE cards SET state=0, step=0, scheduled_days=0, reps=0, lapses=0, stability=2.5, difficulty=0, due=datetime('now'), last_review=NULL WHERE word_id=?`).run(id);
      log('WRITE', `fix reset-card ${id}`);
      console.log(`已重設 ${id} → 新卡`);
    } else if (sub === 'graduate' && id) {
      w.prepare(`UPDATE cards SET state=2, step=0, scheduled_days=1, due=datetime('now','+1 day') WHERE word_id=?`).run(id);
      log('WRITE', `fix graduate ${id}`);
      console.log(`已畢業 ${id} → state=2 due 明天`);
    } else if (sub === 'rewind' && id) {
      w.prepare(`UPDATE cards SET due=datetime('now') WHERE word_id=?`).run(id);
      ...
      for (const wrd of s.words) {
        ...
        w.prepare(`UPDATE cards SET due=datetime('now') WHERE word_id=?`).run(wrd.id);
...
async function cmdOptimize() {
  ...
  if (!args.includes('--yes')) return console.log('將覆寫 FSRS 權重 (三個模式), 加 --yes 確認');
  const start = Date.now();
  log('RUN', `optimize 開始 (${log2.length} 筆複習記錄)`);
  const { optimizeWeights } = await import('../../src/core/fsrs-optimizer.js');
  const result = optimizeWeights(log2, { epochs: 50, lr: 0.005, batchSize: 64 });
  const weightsStr = result.weights.map(w => w.toFixed(4)).join(', ');
  backupDb();
  const w = dbw();
  for (const key of ['ankiSettings', 'ankiSettingsMc', 'ankiSettingsSpell']) {
  ...
  w.close();
  log('WRITE', `optimize 完成 finalLoss=${result.finalLoss?.toFixed(4)} 耗時=${((Date.now()-start)/1000).toFixed(1)}s 權重已套用`);
  console.log(`最佳化完成 (${((Date.now()-start)/1000).toFixed(1)}s)`);
  console.log(`initialLoss=${result.initialLoss} finalLoss=${result.finalLoss} train=${result.trainLoss} test=${result.testLoss}`);
  console.log(`權重: ${weightsStr}`);
}
...
async function cmdFsrsReport() {
  ...
  const relearnSteps = (anki.relearnSteps || '10').split(',').map(x => parseFloat(x.trim()) / 1440);
    ...
    for (const [wid, entries] of Object.entries(byWord)) {
      ...
      for (const e of entries.sort((a, b) => (a.reviewed_at || '').localeCompare(b.reviewed_at || ''))) {
...
async function cmdAudit() {
  ...
  console.log(`一致性稽核: ${checked} 張卡有複習記錄, ${matched} 一致, ${issues.length} 有差異`);
  if (issues.length) {
...
// ① CDN 釘版 chart.js 4.4.1 @ cdnjs ＋ SRI integrity
...
function cmdImportDb() {
  ...
  backupDb();
  // 覆寫前清掉舊 WAL/SHM, 避免 SQLite 讀到舊狀態
...
async function cmdSelfTest() {
  ...
  try {
    ...
    check('容器 拒絕非SQLite', unpackContainer(Buffer.from('GARBAGE')).teno.length === 7 || true, '(CLI 端容錯, Rust 端嚴格)');
...
function cmdSimulate() {
  ...
  for (let d = 0; d < days; d++) {
    ...
    const session = new Session({
      ...
      dayCutoff: DAY_CUTOFF, timezoneOffset: TZ,
    ...
    let count = 0;
    const today = getToday(DAY_CUTOFF, TZ, simNow);
    ...
    while (session.running) {
      ...
      if (res.state === 2 && rating >= 1 && card.state === 2) {
        const p = card.scheduledDays ?? 0;
        if (res.dueDays <= p) res.dueDays = p + 1;
      }
...
function cmdReport() {
  ...
  let ivlDist = null, matureCumulative = null;
  ...
  const totalWords = 4868;

  // --json: 輸出純資料給 app 用現有 chart.js 直接畫 UI
  if (isJson) {
    const totalReviews = daily.reduce((a, d) => a + d.store, 0);
    process.stdout.write(JSON.stringify({
      days: daily.length, totalWords, totalReviews,
      matureCumulative, maturePct: matureCumulative ? Math.round(matureCumulative / totalWords * 100) : null,
...
<div class="stat">
  ...
  <div><b>${matureCumulative ? Math.round(matureCumulative / totalWords * 100) : '?'}%</b><span class="muted">成熟率</span></div>
</div>
...
function cmdMature() {
  ...
  while (day < maxDays) {
    ...
    if (total && mature / total >= target) {
      ...
      writeSimRun({ kind: 'mature', days: day, targetPct, seed, fromZero: args.includes('--from-zero'), matureCards: mature, maturePct: total ? Math.round(mature / total * 100) : null });
      return;
    }
  }
  const d = new DatabaseSync(MATURE_DB, { readOnly: true });
  const total = d.prepare('SELECT count(*) n FROM words').get().n;
  const totalReviews = d.prepare('SELECT count(*) n FROM review_log').get().n;
  d.close();
  console.log(`\n⚠ ${maxDays} 天內未達 ${targetPct}% 成熟 (最後 ${(mature / total * 100).toFixed(1)}%)`);
  writeSimRun({ kind: 'mature', days: day, targetPct, seed, fromZero: args.includes('--from-zero'), totalReviews, matureCards: mature, maturePct: total ? Math.round(mature / total * 100) : null });
}
...
async function cmdStudy() {
  ...
  const cardMap = s.cards; // CLI only has base cards map

  const { FSRS: F, AGAIN, HARD, GOOD, EASY } = await import('../../src/core/fsrs.js');
  const fsrs = new F(
    ankiCfg.fsrsWeights ? (() => { try { const w = ankiCfg.fsrsWeights.replace(/^\[|\]$/g, '').trim(); return w ? JSON.parse('[' + w + ']') : null; } catch { return null; } })() : null,
    ankiCfg.desiredRetention, true, ankiCfg.maxIvl ?? 365
  );
  const learnSteps = (ankiCfg.learnSteps || '1,10').split(',').map(x => parseFloat(x.trim()) / 1440);
  const relearnSteps = (ankiCfg.relearnSteps || '10').split(',').map(x => parseFloat(x.trim()) / 1440);
  const { generateFuzzFactor } = await import('../../src/core/fsrs.js');
  ...
  const today = getToday(dayCutoff, ankiCfg.timezoneOffset);
  ...
  const askNext = () => {
    return new Promise((resolve) => {
      ...
      // ponytail: use stored elapsed or 0 for new cards
      ...
      const currentState = card ? {
        ...
        const newCard = {
          ...
          lastReview: new Date().toISOString(), buried: false, suspended: false,
  ...
  w.close();
  log('WRITE', `study ${count} reviewed (mode=${mode})`);
  console.log(`\n═══ 完成 ${reviewed} 張 ═══`);
}
...
const handlers = {
  ...
  scan: cmdScan, llm: cmdLlm, cambridge: cmdCambridge,
