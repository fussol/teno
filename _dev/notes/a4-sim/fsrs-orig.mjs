...
export class FSRS {
  ...
  review(card, rating, fuzzFactor, learnSteps, relearnSteps, futureCounts) {
    ...
    function hardDelay(steps, stp) {
      ...
      if (stp === 0 && steps.length >= 2) return (steps[0] + steps[1]) / 2;
      return (steps[stp] ?? steps[0] ?? 0);
    }

    // Anki: again_delay → always step[0]
    function againDelay(steps) { return steps[0] ?? (1/1440); }

    if (state === STATE_NEW) {
      if (rating === AGAIN) {
        newState = STATE_LEARNING; newStep = 0; interval = againDelay(actualLearnSteps);
      } else if (rating === HARD) {
        newState = STATE_LEARNING; newStep = 0; interval = hardDelay(actualLearnSteps, 0);
      } else if (rating === GOOD) {
      ...
      if (rating === AGAIN) {
        newStep = 0; interval = againDelay(steps);
      } else if (rating === HARD) {
        interval = hardDelay(steps, stepIdx ?? 0);
      } else if (rating === GOOD) {
        newStep = (stepIdx ?? 0) + 1;
      ...
      if (rating === AGAIN) {
        newLapses++;
        newState = STATE_RELEARNING; newStep = 0;
        interval = againDelay(actualRelearnSteps);
      } else {
        newState = STATE_REVIEW; newStep = 0;
        interval = next_interval(this.w, mem.stability, this.desiredRetention);
      }
