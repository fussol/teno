...
// A4: 轉 learnSteps/relearnSteps 分鐘字串 → 天數陣列。
// NaN/負值/空段丟棄；全丟 → []（fsrs.js review 判 [] 畢業，Anki 空 steps 語意）。
// ''/null/undefined → fallback（呼叫端可傳預設 '1,10'）。0 分鐘步不支援（丟棄 — R1 M-1）。
...
export class FSRS {
  ...
  review(card, rating, fuzzFactor, learnSteps, relearnSteps, futureCounts) {
    ...
    // （rslib steps.rs hard_delay_secs_for_first_step + maybe_round_in_days；2022-12 #2229 起同構）
    //   單 step（首 step 無下一步）：min(1.5x, x+1day) — #2229 加 cap，防 3d step → 4.5d 超過畢業 4d
    //   首 step 有下一步：avg(s0, s1) — 官方無 min cap（查證 #2229 diff 僅 cap 單 step 分支）
    //   其餘：重複目前 step（官方此分支不回 round）
    //   兩個 special case 都過 maybe_round_in_days：>1 天時捨入到整天（單位=天 → DAY=1）
    // A4: 空/無效 steps 回 null（Anki steps.rs Option 語意）→ 呼叫端判 null 畢業
    function hardDelay(steps, stp) {
      if (!steps || steps.length === 0) return null;
      // Anki: secs > DAY → round(secs/DAY)*DAY（此處 steps 單位是天，DAY=1）
      const maybeRoundInDays = (v) => (v > 1) ? Math.round(v) : v;
      let v;
      if (steps.length === 1) {
        v = maybeRoundInDays(Math.min(steps[0] * 1.5, steps[0] + 1));
      } else if (stp === 0) {
        v = maybeRoundInDays((steps[0] + steps[1]) / 2);
      } else {
        v = steps[stp] ?? steps[0] ?? 0;
      }
      return (v == null || !Number.isFinite(v) || v <= 0) ? null : v;
    }

    // Anki: again_delay → always step[0]（空/無效回 null）
