...
export function ensureSession(storeState) {
  ...
  session = new Session({
    ...
    ratedNewToday: storeState.newRatedTodaySpell,
    learnSteps: storeState.ankiSettingsSpell.learnSteps,
    relearnSteps: storeState.ankiSettingsSpell.relearnSteps,
    maxReviewsPerDay: storeState.simParams?.maxReviewsPerDay ?? 0,
    reviewMix: storeState.ankiSettingsSpell.reviewMix ?? 2,
    timezoneOffset: storeState.ankiSettingsSpell.timezoneOffset,
    mode: 'spell',
...
export function ensureQueue(filter, storeState) {
  ...
  if (rolledOver && storeState) {
    ...
    session.ratedNewToday = countNewRatedToday(storeState.reviewLog, today, session.dayCutoff, session.timezoneOffset, 'spell');
  }
...
export function submitAnswer(input) {
  ...
  lastCorrect = userInput.toLowerCase() === session.current.word.word.toLowerCase();
  state = 'ANSWER';
...
let _ttsMod = null;
function getTts() {
  if (!_ttsMod) _ttsMod = import('../lib/tts.js');
  return _ttsMod;
}
...
export function playWord(store) {
  ...
  getTts().then(({ speak }) => {
    speak(word, store.state.ttsSpeed || 0.9, store.state.ttsVoice || 'en-us', store.state.ttsPitch || 50);
  }).catch(() => {});
}
...
export async function rateCard(store, rating, renderFn) {
  ...
  try {
    try {
      await store.actions.rateCard(wid, rating, t, 'spell');
    } catch (err) {
      toast('儲存失敗', 'toast-error');
      return; // C10: 釋鎖交 finally（原手動釋放＋return，語意相同）
    }
    const _goalP = store.actions.incrementGoal('spell').catch(() => {});
...
export async function undoRating(store, renderFn) {
  ...
  _ratingLock = true;
  try {
    const { currentCard, rating, wordId, userInputBefore } = _undoSnapshot;
...
export function mount(store, renderFn) {
  ...
  document.getElementById('spellPlayBtn')?.addEventListener('click', () => playWord(store));
  document.getElementById('spellSubmitBtn')?.addEventListener('click', () => {
  ...
  const h = e => {
    ...
    if (e.key === '[') {
      if (state === 'QUESTION') { e.preventDefault(); playWord(store); }
      return;
    }
    ...
    if (state === 'ANSWER' && e.key >= '1' && e.key <= '4') {
      e.preventDefault();
      rateCard(store, parseInt(e.key) - 1, renderFn);
    }
  };
