...
function bindCardEvents(s, w, idx, total, st) {
  ...
  if (settingsBtn && settingsPop) {
      ...
      cb.addEventListener('change', function() {
        ...
        if (this.checked) {
          ...
          if (st.showComplete) {
            ...
            const cc = document.getElementById('dcsComplete'); if (cc) cc.checked = false;
    ...
    document.getElementById('dcsExampleMax')?.addEventListener('input', function() {
  ...
  if (dk || tk) {
    try {
      ...
      if (f) {
        ...
        if (f.phrases) s(`${prefix}ExampleAppend`, f.phrases);
        return;
      }
    } catch (_) { /* 掉回 LLM */ }
  }
