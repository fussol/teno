...
function showCard(idx) {
  ...
  if (panel) {
    ...
    if (!newBody.dataset._listeners) {
      ...
      newBody.addEventListener('dblclick', onCardBodyClick);
  ...
  return `<div class="card-panel${isFull ? ' full' : ''}" id="cardPreviewModal">
    <div class="card-panel-head">
      ...
      <div class="card-panel-head-actions" style="position:relative">
        ...
        <button title="下一組例句" id="cardExNextBtn">${icon('shuffle')}</button>
...
function bindCardEvents(s, w, st) {
  const body = document.getElementById('cardPreviewBody');
  body.addEventListener('dblclick', onCardBodyClick);
  // TAPFLIP1: 點文字發音——面板掛 document.body（pageContainer 外），發音監聽直接綁面板
  bindSpeakClick(document.getElementById('cardPreviewModal'), () => s.state);
...
function openModal(s, word) {
      ...
      <div class="modal">
        ...
        <div class="form-row" style="${isMobile ? 'flex-direction:column' : ''}">
          ...
          <div class="form-group" style="flex:1">
            ...
            <div style="display:flex;gap:4px"><input class="form-input" id="fSyllables" placeholder="例：dict·io·nary；Enter 跳下一欄" value="${escapeAttr(word?.syllables || '')}" style="flex:1"><button class="btn btn-sm" id="btnFillExtra" type="button" title="韋氏補上音節／字源（片語併入例句；只填空欄）">${icon('sparkle')}</button></div>
          </div>
        </div>
        <div class="form-group">
  ...
  document.getElementById('btnFillExtra')?.addEventListener('click', () => {
    const w = document.getElementById('fWord')?.value.trim();
    if (!w) { toast('請先輸入單字', 'toast-error'); return; }
    mwFillExtra(fSetB, fGetB, w);
  });
  ...
  let _lastAutoFilled = '';
  // ENGINE3: g/s 提升 modal 層（autoFillAll＋音節字源鈕共用；fGet/fSet 避開 modal 參數 s）
  const fGetB = id => {
  ...
  const fSetB = (id, val) => {
    if (!val) return;
    if (id === 'fDefinition') { if (!fGetB('fDefinition')) _defChips.setVal(val); }
    else if (id === 'fExample') { if (!fGetB('fExample')) exChips.setVal(val); }
 ...
 *  getMw 單次快取（三欄一鍵只打一次，同舊）；threshold 預設全開（sparkle 單句模式有無都不擋） */
